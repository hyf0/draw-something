...
} from '@/controller/CanvasController';
...
function Canvas({
  ...
  useEffect(() => {
    const changePlayingUserOff = wsClient.listen(
    ...
    const drawActionOff = wsClient.listen(
  ...
  useEffect(() => {
    ...
    const forbidScroll = (evt: any) => evt.preventDefault();
    canvasContainer.addEventListener('touchmove', forbidScroll, {
      passive: false, // 通知浏览器会调用 preventDefault
    });
    return () => {
      canvasContainer.removeEventListener('touchmove', forbidScroll);
    };
  }, []);
